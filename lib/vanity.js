...
const getDoubleVanityWallet = async (
  ...
  let ts = logAttempts(attempts, inputStart+inputEnd, isChecksum, logFrequency, new Date(Date.now()).getMilliseconds())
  await logPerformance()

  while (!isValidDoubleVanityAddress(wallet.address, inputStart, inputEnd, isChecksum)) {
    if (attempts % logFrequency === 0) {
        ts = logAttempts(attempts, inputStart+inputEnd, isChecksum, logFrequency, ts)
        await logPerformance()
    }
    wallet = getRandomWallet();
    attempts++;
